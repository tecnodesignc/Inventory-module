...
@section('content-header')
    <h1>
        {{ trans('inventory::accounts.title.edit account') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.inventory.account.index') }}">{{ trans('inventory::accounts.title.accounts') }}</a></li>
        <li class="active">{{ trans('inventory::accounts.title.edit account') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.inventory.account.update', $account->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('inventory::admin.accounts.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
